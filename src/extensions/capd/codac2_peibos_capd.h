...
namespace codac2
{
   ...
   * \li A PEIBOS_CAPD_Key representing \f$\mathbf{y}= \sigma(\psi_0(\mathbf{\text{box}}))\f$
   * \li The interval vector \f$\mathbf{z}\f$ containing the image \f$\mathbf{f}(\mathbf{\bar{y}})\f$
   * \li The interval Jacobian matrix \f$mathbf{J}_f\f$ containing \f$\frac{d\mathbf{f}}{d\mathbf{y}}([\mathbf{y}])\f$
   */
   ...
   * 
   * \return A timed map of PEIBOS CAPD results. At each time \f$t\f$, the value is a vector of tuples. Each tuple contains:
   * \li A PEIBOS_CAPD_Key representing \f$\mathbf{y}= \sigma(\psi_0(\mathbf{\text{box}})) + \text{offset}\f$
   * \li The interval vector \f$\mathbf{z}\f$ containing the image \f$\mathbf{f}(\mathbf{\bar{y}})\f$
   * \li The interval Jacobian matrix \f$mathbf{J}_f\f$ containing \f$\frac{d\mathbf{f}}{d\mathbf{y}}([\mathbf{y}])\f$
   */
