...
py::class_<AffineRow> export_AffineRow(py::module& m)
{
  ...
  exported_affinerow_class
    ...
    // Products with a matrix (the result is an affine row vector)

    .def("__mul__", [](const AffineRow& x1, const Matrix& x2) -> AffineRow
        {
          return x1*x2.cast<Affine>();
        },
      py::is_operator())

    .def("__mul__", [](const AffineRow& x1, const IntervalMatrix& x2) -> AffineRow
        {
          return x1*x2.cast<Affine>();
        },
      py::is_operator())

    .def("__mul__", [](const AffineRow& x1, const AffineMatrix& x2) -> AffineRow
        {
          return x1*x2;
        },
      py::is_operator())

    // Scalar products with a column vector (the result is an affine form)

    .def("__mul__", [](const AffineRow& x1, const Vector& x2) -> Affine
        {
    ...
    .def("__mul__", [](const AffineRow& x1, const IntervalVector& x2) -> Affine
        {
          assert_release(x1.size() == x2.size());
          return (x1*x2.cast<Affine>()).eval()(0,0);
        },
    ...
    .def("__mul__", [](const AffineRow& x1, const AffineVector& x2) -> Affine
        {
          assert_release(x1.size() == x2.size());
